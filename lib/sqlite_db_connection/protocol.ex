defmodule Sqlite.DbConnection.Protocol do
  ...
  #       {:ok, %{s | sock: {:gen_tcp, sock}}}
  #     {:error, reason} ->
  #       {:error, Sqlite.DbConnection.Error.exception(tag: :tcp, action: "connect", reason: reason)}
  #   end
  # end
  #
  # ## ssl
  ...
  #     {:ok, <<?N>>} ->
  #       disconnect(s, %Sqlite.DbConnection.Error{message: "ssl not available"}, "")
  #     {:error, reason} ->
  #       err = Sqlite.DbConnection.Error.exception(tag: :tcp, action: "recv", reason: reason)
  #       disconnect(s, err, "")
  #   end
  # end
  #
  ...
  #     {:error, reason} ->
  #       err = Sqlite.DbConnection.Error.exception(tag: :ssl, action: "connect", reason: reason)
  #       disconnect(s, err, "")
  #   end
  # end
  #
  ...
  #       disconnect(s, Sqlite.DbConnection.Error.exception(postgres: fields), buffer)
  #     {:disconnect, _, _} = dis ->
  #       dis
  #   end
  # end
  ...
  #   case Sqlite.DbConnection.TypeServer.fetch(types_key) do
  #     {:ok, table} ->
  ...
  #   version = parameters["server_version"] |> Sqlite.DbConnection.Utils.parse_version
  ...
  #   Sqlite.DbConnection.TypeServer.unlock(ref)
  ...
  #   ## simple queries here are only done by Sqlite.DbConnection.Notifications processes
  #   case msg_recv(s, timeout, buffer) do
  #     {:ok, msg_command_complete(tag: tag), buffer} ->
  #       complete(s, status, %Query{}, [], tag, buffer)
  #     {:ok, msg_error(fields: fields), buffer} ->
  #       sync_recv(s, status, Sqlite.DbConnection.Error.exception(postgres: fields), buffer)
  #     {:ok, msg, buffer} ->
  ...
  #       ok(s, query, buffer)
  #     {:ok, msg_error(fields: fields), buffer} ->
  #       exception = Sqlite.DbConnection.Error.exception(postgres: fields)
  ...
  #   case Sqlite.DbConnection.Error.exception(postgres: fields) do
  #     %Sqlite.DbConnection.Error{postgres: %{code: :invalid_sql_statement_name}} ->
  ...
  #       sync_recv(s, status, %Sqlite.DbConnection.Result{}, buffer)
  #     {:ok, msg_error(fields: fields), buffer} ->
  #       sync_recv(s, status, Sqlite.DbConnection.Error.exception(postgres: fields), buffer)
  #     {:ok, msg, buffer} ->
  ...
  #   result =  %Sqlite.DbConnection.Result{command: command, num_rows: nrows || 0}
  ...
  #       ok(s, result, buffer)
  #     {:ok, msg_error(fields: fields), buffer} ->
  #       ok(s, Sqlite.DbConnection.Error.exception(postgres: fields), buffer)
  #     {:ok, msg, buffer} ->
  ...
  #       err = Sqlite.DbConnection.Error.exception(postgres: fields)
  #       sync_recv(s, status, err, buffer)
  #     {:ok, msg_error(fields: fields), buffer} ->
  #       err = Sqlite.DbConnection.Error.exception(postgres: fields)
  ...
  #       _ = Sqlite.DbConnection.Parameters.put(parameters, name, value)
  #       s
  #     is_map(parameters) ->
  ...
  # end
  #
  # defp ok(s, %Sqlite.DbConnection.Result{} = res, buffer) do
  #   {:ok, res, %{s | buffer: buffer}}
  # end
  # defp ok(s, %Sqlite.DbConnection.Query{} = query, buffer) do
  #   {:ok, query, %{s | buffer: buffer}}
  #  end
  # defp ok(s, nil, buffer) do
  #   {:ok, %{s | buffer: buffer}}
  # end
  # defp ok(s, %Sqlite.DbConnection.Error{} = err, buffer) do
  ...
  #   err = Sqlite.DbConnection.Error.exception(tag: tag, action: action, reason: reason)
  #   disconnect(s, err, buffer)
  # end
  #
  ...
  #   err = %Sqlite.DbConnection.Error{message: "unexpected postgres status: #{postgres}"}
  #   {:disconnect, err, s}
  # end
  #
  ...
  #   Sqlite.DbConnection.Parameters.delete(ref)
  # end
