defmodule Sqlite.DbConnection.Protocol do
  ...
  defp sqlite_error({:error, {sqlite_errcode, message}}, s) do
    {:error,
     %Sqlite.DbConnection.Error{sqlite: %{code: sqlite_errcode}, message: to_string(message)}, s}
  end
  ...
  defp run_stmt(query, params, opts, s) do
    ...
    case query_rows(s.db, to_string(query), query_opts) do
      {:ok, %{rows: raw_rows, columns: raw_column_names}} ->
        {rows, num_rows, column_names} =
          case {raw_rows, raw_column_names} do
            {_, []} -> {nil, get_changes_count(s.db, command), nil}
            _ -> {raw_rows, length(raw_rows), raw_column_names}
          end

        {:ok,
         %Sqlite.DbConnection.Result{
           rows: rows,
           num_rows: num_rows,
      ...
      {:error, :wrong_type} ->
        {:error, %ArgumentError{message: "Wrong type"}, s}
      ...
      {:error, :args_wrong_length} ->
        {:error, %ArgumentError{message: "parameters must match number of placeholders in query"},
         s}
    end
  end

  defp get_changes_count(db, command)
       when command in [:insert, :update, :delete] do
  ...
  defp command_from_words([verb, subject, _])
       when verb == "alter" or verb == "create" or verb == "drop" do
  ...
  defp handle_transaction(stmt, opts, s) do
    ...
    result = %Sqlite.DbConnection.Result{rows: nil, num_rows: nil, columns: nil, command: command}
    {:ok, result, s}
  end
