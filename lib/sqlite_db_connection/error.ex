defmodule Sqlite.DbConnection.Error do
  ...
  def exception([tag: :ssl, action: action, reason: :timeout]) do
    %Sqlite.DbConnection.Error{message: "ssl #{action}: timeout"}
  end

  def exception([tag: :ssl, action: action, reason: reason]) do
    formatted_reason = :ssl.format_error(reason)
    %Sqlite.DbConnection.Error{message: "ssl #{action}: #{formatted_reason} - #{inspect(reason)}"}
  end
  ...
  def exception([tag: :tcp, action: action, reason: reason]) when not reason in @nonposix_errors do
    ...
    %Sqlite.DbConnection.Error{message: "tcp #{action}: #{formatted_reason} - #{inspect(reason)}"}
  end

  def exception([tag: :tcp, action: action, reason: reason]) do
    %Sqlite.DbConnection.Error{message: "tcp #{action}: #{reason}"}
  end
